...
check test tests: fmt lint $(GENERATED) | $(GOTESTSUM) ; $(info $(M) running $(NAME:%=% )tests…) @ ## Run tests
	$Q mkdir -p test
	$Q $(GOTESTSUM) --junitfile TEST-junit.xml -- -timeout $(TIMEOUT)s $(ARGS) $(PKGS)
...
test-coverage: | $(GOCOV) $(GOCOVXML) $(GOTESTSUM) ; $(info $(M) running coverage tests…) @ ## Run coverage tests
	$Q mkdir -p test/coverage/
	$Q $(GOTESTSUM) -- \
		-coverpkg=$(shell echo $(PKGS) | tr ' ' ',') \
		-covermode=$(COVERAGE_MODE) \
		-coverprofile=test/coverage/profile.out $(PKGS)
	$Q $(GO) tool cover -html=test/coverage/profile.out -o test/coverage/coverage.html
	$Q $(GOCOV) convert test/coverage/profile.out | $(GOCOVXML) > test/coverage/coverage.xml
	@echo -n "Code coverage: "; \
		echo "scale=1;$$(sed -En 's/^<coverage line-rate="([0-9.]+)".*/\1/p' test/coverage/coverage.xml) * 100 / 1" | bc -q
